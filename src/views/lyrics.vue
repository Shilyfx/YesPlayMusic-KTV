<template>
  <transition name="slide-up">
    ...
    >
      ...
      <div class="lyric-sync-quick-control" role="group" aria-label="歌词同步">
        <span>歌词 {{ lyricOffsetLabel }}</span>
        <button
          type="button"
          aria-label="歌词延迟 0.1 秒"
          title="歌词延迟 0.1 秒"
          @click="adjustLyricOffset(-0.1)"
        >
          −
        </button>
        <button
        ...
        <button type="button" @click="setLyricOffset(0)">归零</button>
      </div>
    </div>
  </transition>
</template>
...
export default {
  ...
  computed: {
    ...
    lyricOffsetLabel() {
      ...
      return `${this.lyricOffsetSeconds > 0 ? '提前' : '延迟'} ${Math.abs(
        this.lyricOffsetSeconds
      ).toFixed(1)}s`;
    },
    noLyric() {
  ...
  methods: {
    ...
    adjustLyricOffset(amount) {
      this.setLyricOffset(this.lyricOffsetSeconds + amount);
    },
    ...
    setLyricsInterval() {
      this.lyricsInterval = setInterval(() => {
        ...
        // Positive offset means a line is highlighted before its source time.
        // Source timestamps and click-to-seek continue to use their original time.
        const displayProgress = progress + this.lyricOffsetSeconds;
        ...
        this.highlightLyricIndex = this.lyric.findIndex((l, index) => {
          ...
          return (
            displayProgress >= l.time &&
            (nextLyric ? displayProgress < nextLyric.time : true)
          );
        });
